...
public:
  ...
  */
  ...
  /*!
     ...
     value other than document root.
      \param allocator Allocator for creating the values if the specified value
     or its parents are not exist.
      \param alreadyExist If non-null, it stores whether the resolved value is
     already exist.
      \return The resolved newly created (a JSON Null value), or already exists
     value.
  */
    ...
    for (const Token *t = tokens_; t != tokens_ + tokenCount_; ++t) {
      if (v->IsArray() && t->name[0] == '-' && t->length == 1) {
        ...
        if (v->IsArray()) {
          ...
          if (m == v->MemberEnd()) {
            v->AddMember(Value(t->name, t->length, allocator).Move(),
  ...
  */
  ...
  /*!
     ...
     parents and clone the default value.
...
private:
  ...
  */
  ...
  /*!
     ...
     False for string representation.
      \tparam OutputStream type of output stream.
      \param os The output stream.
  */
